...
let msg =``;
let isSign = false;
let withdrawsitemId = '1'; //每天额度是每天刷新的10点吧
...
async function start() {
    ...
    await $.wait(2000);
    await getServiceAreaTaskList(2 * 1000);
    await $.wait(2000);
    await getChapterTaskList(2 * 1000);
    await $.wait(2000);
    await list(2 * 1000);
    await $.wait(2000);
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 100) {
                    memberinfo = result.data.userInfoBO.clientInfo.user.nickname
                    DoubleLog(`\n 查询会员: ✅ ，用户：${memberinfo} `)
                } else {
                    DoubleLog(`\n 查询会员: ❌ ，原因是：${result.msg}`)
                }
...
function getMemberScore(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/api/task/v1/wallet/user/info`,
            headers: {
                Host: host,
				'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
				'Cookie': 'sessionid=' + ck[0],
            ...
            body: `encrypted_param=gWOF3VeHkJHLuXYgN9aIDg/0VU2O/xX4Zd7DwZfkcQV2sT6mzmxNgrCtJkMxCJEZOzULMbzvnxIYSrKKR5bVeXelDZI5LLW212ayFtXqGi9VAourxZa3HuXtvDKML63GaW8ovHaNsaS+eCGRWZieOHilqx2XHSax//TKptXtT4Y=&sign=07e1b8abd84d40a1a8dd86f0bb1d405e&`,
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 100) {
                    DoubleLog(`\n 收益查询: ✅ ，今日 ${Number(result.data.today)} 金币，总 ${Number(result.data.total)} 金币 ~ ${result.data.cash} 元`)
                } else {
                    DoubleLog(`\n 收益查询: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
 ...
 * @returns {Promise<unknown>}
 */
function getServiceAreaTaskList(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/task_api/task/getServiceAreaTaskList`,
            headers: {
 ...
 * @returns {Promise<unknown>}
 */
function getChapterTaskList(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
...
function list(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            ...
            body: `sign=75F761FEACAA604E19A0F4FEFABE556E&time=1654363935000`,
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
 ...
 * @returns {Promise<unknown>}
 */
function finish(taskId ,timeout = 2000) {
    return new Promise((resolve) => {
...
function withdraw(timeout = 2000) {
    return new Promise((resolve) => {
        sign =md5('7b7fpld4roey0e6e&itemId='+withdrawsitemId+'&platform=0&time='+timestamp)
        let url = {
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_xfkd_Cookie`)
        return;
    }
    return true;
}
